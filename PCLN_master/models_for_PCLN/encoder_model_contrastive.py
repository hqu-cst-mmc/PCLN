...
class encoder_edtcn(nn.Module):
    ...
    def __init__(self):
        ...
        # self.channel_norm1 = Lambda(lambda x: x / (torch.max(torch.abs(x), 2, keepdims=True) + 1e-5))
        self.pool1 = nn.MaxPool1d(kernel_size=2)

        self.conv2 = nn.Conv1d(256, 512, kernel_size=1)
        self.drop2 = SpatialDropout(0.3)
        # self.channel_norm2 = Lambda(lambda x: x / (torch.max(torch.abs(x), 2, keepdims=True) + 1e-5))
